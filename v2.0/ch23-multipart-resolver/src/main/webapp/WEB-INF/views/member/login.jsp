...
<script src="${cp}/resources/js/member/login.js" defer></script>
</head>
<body>
	...
	<form id="tries" name="tries" method="post" action="${cp}/member/signup" style="display: none">
		 <input type="hidden" id="triesId"  name="id" value="${ loginFailId }"/>
 		 <input type="hidden" id="triesCurpwd" name="curpwd" value="${ loginFailPwd }"/>
	</form>
	...
	<%			
		...
		while(attributeNames.hasMoreElements()) {
			...
			if(key.equals("signup")) {
				if(value != null) {
					if(value.toString().equals("signup-success")) {
				...
				if(value != null) {
					if(value.toString().equals("delete-success")) {
				...
				if(value != null) {
					if(value.toString().equals("login-fail")) {
					...
					} else if(value.toString().equals("logout")) {
						out.println("<script>alert('[LOGOUT INFO] LOGOUT SUCCESS!')</script>");
						session.removeAttribute("login");
					} else if(value.toString().equals("session-end")) {
	...
	%>
	<h1>Login</h1>
	<form id="login" name="login" method="post" action="${cp}/member/doLogin">
		<label for="id">ID: </label>
		<input type="text" id="loginId" name="mId" maxlength="20" autocomplete="off"><br/>
		<label for="curpwd">PASSWORD: </label> 
		<input type="password" id="loginCurpwd" name="mCurpwd" maxlength="20" autocomplete="off"><br/>
		<input type="submit" value="login">
	</form>
	<a href="${cp}/member/signup"><button>signup</button></a>
