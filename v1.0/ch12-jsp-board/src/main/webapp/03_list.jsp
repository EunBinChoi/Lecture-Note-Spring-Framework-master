...
<%	
	  ...
	  // 검색을 위해서 호출받은 keyWord() 값이 null 값이 아니면 
	  // 요청받은 keyWord 값과 keyField 값을 받아서 검색 처리
	  if (request.getParameter("keyWord") != null) {
		  keyWord = request.getParameter("keyWord");
		  keyField = request.getParameter("keyField");
	  }
	  
	  // 검색을 하고 다른 모든 리스트를 보기 위해서 [처음으로]를 클릭하면 
	  // form에 hidden 타입의 reload 값을 true로 요청하여 검색을 위해서 선언한 keyWord, keyField 값을 공백으로 처리
	...
	<table align="center" width="600" cellpadding="3">
		<tr>
			...
			<%
			      ...
			      // 전체 게시물을 리턴하기 위해 getBoardList() 호출
				  // start ~ end : 한 페이지당 출력할 게시물의 개수를 지정
				  // 검색을 위해 넘겨받은 keyWord, keyField
				  // 만약 검색하지 않고 전체 게시물을 리턴한다면 keyWord, keyField은 공백임
			...
			%>
				  <table width="100%" cellpadding="2" cellspacing="0">
					...
					<tr>
						...
						<td>
						<%
							  // 만약 가져온 게시물이 답변 글이라면 (depth > 0)
							  // 제목 앞에 스페이스 (&nbsp;) 값을 주어 약간 뒤쪽에 제목을 출력하여 답변 글을 표시
							  if(depth > 0){
								// 댓글 깊이만큼 반복하면서
